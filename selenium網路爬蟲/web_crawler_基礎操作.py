...
print(type(browser))
target_urls = [
    'C:\\Users\\sean\\Desktop\\python授課資料(基礎)\\selenium_pandas_project\\index.html']
...
for idx, url in enumerate(target_urls, start=1):
    ...
    tag0 = browser.find_element(
        by=By.XPATH, value='//body/section/p')  # 可找到符合指定相對Xpath的第一個元素內容
    # 當有重複元素的時候以索引指定(注意這邊索引編號的方式)
    tag1 = browser.find_element(by=By.XPATH, value='//body/section/p[1]')
    # 當有重複元素的時候以索引指定(注意這邊索引編號的方式)
    tag2 = browser.find_element(by=By.XPATH, value='//body/section/p[2]')
    tag3 = browser.find_element(
        by=By.XPATH, value="//body/section/p[@class='price']")  # 以元素屬性指定
    pic = browser.find_element(
        by=By.XPATH, value="//body/section/img[@class='picture']")  # 以元素屬性指定照片
    print("指定XPATH找到的文字:\n{}".format(tag1.text))
    print("指定XPATH找到的文字:\n{}".format(tag2.text))
